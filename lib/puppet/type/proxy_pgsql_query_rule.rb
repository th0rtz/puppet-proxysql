...
Puppet::Type.newtype(:proxy_pgsql_query_rule) do
  @doc = 'Manage a ProxySQL pgsql_query_rules entry.'
  ...
  validate do
    ...
    raise('name must match \'pgsql_query_rule-\'<rule_id> format') if (self[:ensure] == :present) && self[:name] != "pgsql_query_rule-#{self[:rule_id]}"
  end
  ...
  newproperty(:digest) do
    desc 'match queries with a specific digest, as returned by stats_pgsql_query_digest.digest'
    newvalue(%r{\w+})
  end
