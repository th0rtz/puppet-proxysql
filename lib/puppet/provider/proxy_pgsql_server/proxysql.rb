...
Puppet::Type.type(:proxy_pgsql_server).provide(:proxysql, parent: Puppet::Provider::Proxysql) do
  ...
  def self.instances
    ...
    servers = mysql([defaults_file, '-NBe',
                     'SELECT `hostname`, `port`, `hostgroup_id` FROM `pgsql_servers`'].compact).split(%r{\n})
  ...
  # We iterate over each proxy_pgsql_server entry in the catalog and compare it against
  ...
  def create
    ...
    query = 'INSERT INTO pgsql_servers (`hostname`, `port`, `hostgroup_id`, `status`, `weight`, `compression`,  ' \
            '`max_connections`, `max_replication_lag`, `use_ssl`, `max_latency_ms`, `comment`) ' \
  ...
  def destroy
    ...
    query = 'DELETE FROM `pgsql_servers` ' \
            "WHERE `hostname` =  '#{hostname}' AND `port` = #{port} AND `hostgroup_id` = '#{hostgroup_id}'"
    mysql([defaults_file, '-e', query].compact)
  ...
  def flush
    ...
    mysql([defaults_file, '-NBe', 'LOAD PGSQL SERVERS TO RUNTIME'].compact) if load_to_runtime == :true

    save_to_disk = @resource[:save_to_disk]
    mysql([defaults_file, '-NBe', 'SAVE PGSQL SERVERS TO DISK'].compact) if save_to_disk == :true
  end
  ...
  def update_server(properties)
    ...
    query = "UPDATE pgsql_servers SET #{values.join(', ')} " \
            "WHERE `hostname` =  '#{hostname}' AND `port` = #{port} AND `hostgroup_id` = '#{hostgroup_id}'"
    mysql([defaults_file, '-e', query].compact)
