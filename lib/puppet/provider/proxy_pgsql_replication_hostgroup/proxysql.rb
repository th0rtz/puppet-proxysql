...
Puppet::Type.type(:proxy_pgsql_replication_hostgroup).provide(:proxysql, parent: Puppet::Provider::Proxysql) do
  ...
  def self.instances
    ...
    hostgroups = mysql([defaults_file, '-NBe',
                        'SELECT `writer_hostgroup`, `reader_hostgroup`, `comment` FROM `pgsql_replication_hostgroups`'].compact).split(%r{\n})
  ...
  # We iterate over each proxy_pgsql_replication_hostgroup entry in the catalog and compare it against
  ...
  def create
    ...
    query = 'INSERT INTO `pgsql_replication_hostgroups` (`writer_hostgroup`, `reader_hostgroup`, `comment`) ' \
  ...
  def destroy
    ...
    query = 'DELETE FROM `pgsql_replication_hostgroups` ' \
  ...
  def comment=(value)
    writer_hostgroup = @resource.value(:writer_hostgroup)
    reader_hostgroup = @resource.value(:reader_hostgroup)
    query = "UPDATE pgsql_replication_hostgroups SET `comment` = '#{value}' " \
            "WHERE `writer_hostgroup` =  #{writer_hostgroup} AND `reader_hostgroup` = #{reader_hostgroup}"
    mysql([defaults_file, '-e', query].compact)
