...
Puppet::Type.type(:proxy_pgsql_user).provide(:proxysql, parent: Puppet::Provider::Proxysql) do
  ...
  def self.instances
    users = mysql([defaults_file, '-NBe',
                   'SELECT username FROM pgsql_users'].compact).split(%r{\n})

    # To reduce the number of calls to MySQL we collect all the properties in
    # one big swoop.
    users.map do |name|
      query = "SELECT password, active, use_ssl, default_hostgroup, default_schema, schema_locked, transaction_persistent, fast_forward, backend, frontend, max_connections FROM pgsql_users WHERE username = '#{name}'"
  ...
  # We iterate over each proxy_pgsql_user entry in the catalog and compare it against
  ...
  def create
    ...
    query = 'INSERT INTO pgsql_users (`username`, `password`, `active`, `use_ssl`, `default_hostgroup`, `default_schema`,  ' \
  ...
  def destroy
    ...
    mysql([defaults_file, '-e', "DELETE FROM pgsql_users WHERE username = '#{name}'"].compact)
  ...
  def flush
    ...
    mysql([defaults_file, '-NBe', 'LOAD PGSQL USERS TO RUNTIME'].compact) if load_to_runtime == :true

    save_to_disk = @resource[:save_to_disk]
    mysql([defaults_file, '-NBe', 'SAVE PGSQL USERS TO DISK'].compact) if save_to_disk == :true
  end
  ...
  def update_user(properties)
    ...
    query = "UPDATE pgsql_users SET #{values.join(', ')} WHERE username = '#{name}'"
