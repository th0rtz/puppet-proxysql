...
Puppet::Type.type(:proxy_pgsql_query_rule).provide(:proxysql, parent: Puppet::Provider::Proxysql) do
  ...
  def self.instances
    ...
    rules = mysql([defaults_file, '-NBe',
                   'SELECT `rule_id` FROM `pgsql_query_rules`'].compact).split(%r{\n})
    ...
    rules.map do |rule_id|
      query = 'SELECT `active`, `username`, `schemaname`, `flagIN`, `flagOUT`, `apply`,  ' \
              ...
              "FROM `pgsql_query_rules` WHERE rule_id = '#{rule_id}'"
  ...
  # We iterate over each proxy_pgsql_query_rule entry in the catalog and compare it against
  ...
  def create
    ...
    query = 'INSERT INTO `pgsql_query_rules` (' \
  ...
  def destroy
    ...
    mysql([defaults_file, '-e', "DELETE FROM `pgsql_query_rules` WHERE `rule_id` = '#{rule_id}'"].compact)
  ...
  def update_query_rule(properties)
    ...
    query = "UPDATE `pgsql_query_rules` SET #{values.join(', ')} WHERE `rule_id` = '#{rule_id}'"
